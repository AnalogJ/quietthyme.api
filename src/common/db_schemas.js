...
module.exports = {
    createUser: function(user){
        if(!validateCreateUser(user)){
            throw new Error("User is invalid: " + JSON.stringify(validateCreateUser.errors))
        }
        return user
    },
    createBook: function(book){
        if(!validateCreateBook(book)){
            throw new Error("Book is invalid: " + JSON.stringify(validateCreateBook.errors))
        }
        return book
    },
    createCredential: function(cred){
        if(!validateCreateCredential(cred)){
            throw new Error("Cred is invalid: " + JSON.stringify(validateCreateCredential.errors))
        }
        return cred
    },
    updateUser: function(user){
        if(!validateUpdateUser(user)){
            throw new Error("User is invalid: " + JSON.stringify(validateUpdateUser.errors))
        }
        return user
    },
    updateBook: function(book){
        if(!validateUpdateBook(book)){
            throw new Error("Book is invalid: " + JSON.stringify(validateUpdateBook.errors))
        }
        return book
    },
    updateCredential: function(cred){
        if(!validateUpdateCredential(cred)){
            throw new Error("Cred is invalid: " + JSON.stringify(validateUpdateCredential.errors))
        }
        return cred
    }
}