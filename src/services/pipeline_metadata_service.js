...
PipelineMetadataService.flatten_data_sets = function(
  current_sources,
  raw_data_sets_promises
) {
  ...
  raw_data_sets_promises.forEach(function(result) {
    if (result.state === 'fulfilled') {
      ...
      console.error(
        'Skipping data_set which finished processing with an error: ' +
          result.reason
      );
    }
  });
  ...
  data_sets.forEach(function(data_set) {
    ...
    for (var prop in data_set) {
      var current_prop_type =
        Constants.metadata_data_set_types[current_sources[prop] || ''];
      ...
      if (prop == 'image') {
        ...
        parsed_book[prop] = (parsed_book[prop] || [])
          .concat(data_set[prop] || []);
      } else if (
        (!current_prop_type ||
          data_set_type.priority < current_prop_type.priority) &&
        data_set[prop]
      ) {
...
PipelineMetadataService.generate_existing_filename_ids = function(
  filename_ids
) {
  return { filename_ids: filename_ids, _type: 'file' };
};

PipelineMetadataService.generate_embedded_opf_data_set = function(
  opf_metadata_path
) {
  return ParseExternalService.read_opf_file(opf_metadata_path).then(function(
    parsed_book
  ) {
...
// Goodreads Data
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
PipelineMetadataService.generate_goodreads_data_set = function(
  goodreads_book_id
) {
  ...
  return provider
    ...
    .then(function(goodreads_book) {
      var parsed_book = ParseExternalService.parse_goodreads_book_details(
        goodreads_book
      );
...
PipelineMetadataService.generate_goodreads_data_set_by_title_author = function(
  title,
  author
) {
  title = title.replace(/-/g, '');
  author = author.replace(/-/g, '');
  var clean_title_author = (title + ' - ' + author)
    .replace(/ *\([^)]*\) */g, '')
    .replace(/ *\[[^\]]*\] */g, '');
  debug(
    'Searching for Goodreads book id by title and author: %s',
    clean_title_author
  );
  var goodreads = require('goodreads.js');
  var provider = new goodreads.provider({
  ...
  return provider
    ...
    .then(function(data) {
      return data
        .GoodreadsResponse.search[0].results[0].work[0].best_book[0].id[0]['_'];
    })
    .then(function(goodreads_id) {
      debug('Found Goodreads book id: %s', goodreads_id);
      return PipelineMetadataService.generate_goodreads_data_set(goodreads_id);
...
// OPF Data
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
PipelineMetadataService.generate_opf_data_set = function(
  opf_metadata,
  metadata_storage_cloud_file,
  storage_type
) {
