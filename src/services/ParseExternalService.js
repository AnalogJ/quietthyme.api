...
ParseExternalService.merge_parsed_data = function(book_default, additional_data) {
    ...
    if(book_default.short_summary){delete additional_data.short_summary}
...
ParseExternalService.generate_opf_from_book = function(details){
    ...
    if (details.barnesnoble_id) opf_doc.identifiers["BARNESNOBLE"] = {value: details.barnesnoble_id, scheme: "BARNESNOBLE", id:null};
    if (details.isbn || details.isbn10) opf_doc.identifiers["ISBN"] = {value: details.isbn || details.isbn10, scheme:"ISBN", id:null};
    ...
    if (details.average_rating) opf_doc.metadata['calibre:rating'] = details.average_rating;
    if (details.series_name) opf_doc.metadata['calibre:series'] = details.series_name;
    if (details.series_number) opf_doc.metadata['calibre:series_index'] = details.series_number;
...
ParseExternalService.parse_goodreads_search_results = function (goodreads_response) {
    ...
    if(!(goodreads_response.GoodreadsResponse.search && goodreads_response.GoodreadsResponse.search[0].results && goodreads_response.GoodreadsResponse.search[0].results[0] && goodreads_response.GoodreadsResponse.search[0].results[0].work && goodreads_response.GoodreadsResponse.search[0].results[0].work[0] )){
        ...
        parsed_results = goodreads_response.GoodreadsResponse.search[0].results[0].work.map(function (item) {
            return {
...
//    }
    ...
    parsed_book.published_date = new Date((goodreads_book.publication_year[0] | 0), (goodreads_book.publication_month[0] | 0), (goodreads_book.publication_day[0] | 0));
    //generate tags
    //sort them
    ...
    parsed_book.tags = goodreads_book.popular_shelves[0].shelf
        .reduce(function (prev, current, array) {
...
ParseExternalService.parse_goodreads_shelf_content = function(response){
    ...
    var parsed_shelf_content = goodreads_shelf_content.review.map(function(goodreads_shelf_book){
        var parsed_shelf_book = {};
        //parse book
        parsed_shelf_book['date_added'] = new Date(goodreads_shelf_book['date_added'][0]);
        if(goodreads_shelf_book.book[0].isbn13 && typeof goodreads_shelf_book.book[0].isbn13[0] == "string"){
            parsed_shelf_book.isbn = goodreads_shelf_book.book[0].isbn13[0];
        }
        if(goodreads_shelf_book.book[0].isbn && typeof goodreads_shelf_book.book[0].isbn[0] == "string") {
