...
PipelineService.create_with_pipeline = function(
  primary_criteria,
  metadata_pipeline,
  image_pipeline,
  opts
  ...
  var primary_dataset =
    _.find(metadata_pipeline, { _type: 'api' }) ||
  ...
  if (primary_dataset) {
    ...
    if (
      !(
        primary_dataset.isbn ||
        primary_dataset.isbn10 ||
        primary_dataset.goodreads_id
      ) &&
      !(
        primary_dataset.short_summary &&
        primary_dataset.tags &&
        primary_dataset.tags.length &&
        primary_dataset.cover
      ) &&
      (primary_dataset.title &&
        primary_dataset.authors &&
        primary_dataset.authors.length)
    ) {
      ...
      metadata_pipeline.push(
        PipelineMetadataService.generate_goodreads_data_set_by_title_author(
          primary_dataset.title,
          primary_dataset.authors[0]
        )
      );
    }
  }
    ...
    bookPromise
      .then(function(book) {
        ...
        if (book.goodreads_id) {
          metadata_pipeline.push(
            PipelineMetadataService.generate_goodreads_data_set(
              book.goodreads_id
            )
          );
        } else if (book.isbn || book.isbn10) {
          metadata_pipeline.push(
            PipelineMetadataService.generate_goodreads_data_set_by_isbn(
        ...
        return (q
            ...
            .then(function(processed_data_sets) {
              ...
              return PipelineMetadataService.flatten_data_sets(
                book.sources || {},
                processed_data_sets
              );
            }) );
      ...
      .spread(function(current_sources, flattened_data, add_image_pipeline) {
        ...
        if (flattened_data.isbn || flattened_data.isbn10) {
          image_pipeline.push(
            PipelineImageService.generate_openlibrary_data_set(
              flattened_data.isbn || flattened_data.isbn10
            )
          );
          image_pipeline.push(
            PipelineImageService.generate_amazon_data_set(
              flattened_data.isbn || flattened_data.isbn10
            )
          );
        }
        ...
        var externalImagePromise = qCombinators
          .fallback(
            PipelineImageService.process_image_pipeline(
              current_sources,
              image_pipeline
            )
          )
          .then(function(resp) {
            if (resp.data) {
              return bookPromise.then(function(book) {
                ...
                var image_key = StorageService.create_content_identifier(
                  'cover',
                  book.user_id,
                ...
                return StorageService.upload_file_from_stream(
                  resp.data,
                  '.jpg',
...
// Private functions
function _populate_book_with_parsed_data(
  bookPromise,
  sourcesPromise,
  parsed_bookPromise,
  coverPromise
  ...
  if (coverPromise.state === 'fulfilled' && coverPromise.value) {
    parsed_book.cover =
      coverPromise.value.bucket + '/' + coverPromise.value.key;
  }
  ...
  final_book.short_summary = final_book.short_summary
    ? toMarkdown(final_book.short_summary, {
        converters: [
          {
            filter: 'div',
  ...
  if (final_book.published_date instanceof Date) {
    final_book.published_date = Utilities.ISODateString(
      final_book.published_date
    );
  }
  if (final_book.last_modified instanceof Date) {
    final_book.last_modified = Utilities.ISODateString(
      final_book.last_modified
    );
  }
