...
function web_endpoint() {
  return (
    'https://' +
    (nconf.get('STAGE') == 'master' ? 'www' : 'beta') +
    '.quietthyme.com'
  );
}
...
module.exports.acquisition_feed = function acquisition_feed(
  token,
  id,
  current_path,
  next_path,
...
module.exports.navigation_feed = function acquisition_feed(
  token,
  id,
  current_path,
  next_path,
...
module.exports.search_description_feed = function search_description_feed(
  token
) {
  return {
    ...
    url: {
      template:
        ...
        '/search?query={searchTerms}&amp;page={startPage?}',
    },
  };
};
...
module.exports.generatePaginatedSeriesQuery = function(
  db_client,
  user_id,
  limit,
  page
    ...
    links: [
      {
        ...
        href:
          token_endpoint(token) +
          '/in_series/' +
          Base64Service.urlEncode(series_name),
      },
    ],
  };
};
...
function bookToBaseEntry(id, token, book) {
  ...
  if (book.storage_format) {
    storage_extension =
      book.storage_format[0] == '.'
        ? book.storage_format.substr(1)
        : book.storage_format;
  }
      ...
      ? book.authors.map(function(author) {
          return {
            ...
            uri:
              token_endpoint(token) +
              '/by_author/' +
              Base64Service.urlEncode(author),
          };
        })
      : [],
...
module.exports.bookToFullEntry = function(id, token, book, path) {
  ...
  if (book.series_name) {
    opds_entry.links.push({
      ...
      href:
        token_endpoint(token) +
        '/in_series/' +
        Base64Service.urlEncode(book.series_name),
  ...
  book.authors.forEach(function(author) {
    opds_entry.links.push({
      type: 'application/atom+xml;profile=opds-catalog;kind=acquisition',
      href:
        token_endpoint(token) + '/by_author/' + Base64Service.urlEncode(author),
