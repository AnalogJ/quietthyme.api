...
var callback_url = "https://api.quietthyme.com/link/callback/";
...
module.exports = {
    ...
    callback: function(event, context, cb){
        ...
        if (event.path.serviceType == 'dropbox') {
            ...
            storage_client = new cloudrail.services.GoogleDrive(//connect url (browser must redirect here)
                authenticator
                , process.env.OAUTH_GOOGLE_CLIENT_KEY, process.env.OAUTH_GOOGLE_CLIENT_SECRET, callback_url + event.path.serviceType ,undefined);
        }
        else if (event.path.serviceType == 'box') {
            storage_client = new cloudrail.services.Box(//connect url (browser must redirect here)
                authenticator
                , process.env.OAUTH_BOX_CLIENT_KEY, process.env.OAUTH_BOX_CLIENT_SECRET, callback_url + event.path.serviceType ,undefined);
        }
        else if (event.path.serviceType == 'skydrive') {
            storage_client = new cloudrail.services.OneDrive(//connect url (browser must redirect here)
                authenticator
                , process.env.OAUTH_SKYDRIVE_CLIENT_KEY, process.env.OAUTH_SKYDRIVE_CLIENT_SECRET, callback_url + event.path.serviceType ,undefined);
        }
        ...
        DBService.get()
            ...
            .then(function(db_row_data){
                ...
                storage_client.resumeLogin(db_row_data.auth_data, function(res_err){
