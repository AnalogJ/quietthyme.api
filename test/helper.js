...
before(function(done) {
  ...
  var cfData = yaml.safeLoad(
    fs.readFileSync('./cloudformation-resources.yaml', 'utf8')
  );
  ...
  credsTable.TableName =
    'quietthyme-api-' + nconf.get('STAGE') + '-credentials';
...
function removeSensitiveData(rawString) {
  ...
  if (process.env.OAUTH_GOODREADS_CLIENT_KEY) {
    rawString = rawString.replace(
      new RegExp(process.env.OAUTH_GOODREADS_CLIENT_KEY, 'g'),
  ...
  if (process.env.OAUTH_GOODREADS_CLIENT_SECRET) {
    rawString = rawString.replace(
      new RegExp(process.env.OAUTH_GOODREADS_CLIENT_SECRET, 'g'),
