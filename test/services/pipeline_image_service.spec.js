...
describe('ImagePipelineService', function () {
    ...
    describe('#download_cover_by_isbn()', function () {
        ...
        it('Should fail when downloading an invalid image from amazon', function (done) {
            var data_set = PipelineImageService.generate_amazon_data_set('0971633896');
            PipelineImageService.generate_download_cover_promise(data_set.url, data_set._type)
                .fail(function(err){
        ...
        it('Should download an image from amazon', function (done) {
            var data_set = PipelineImageService.generate_amazon_data_set('0971633894');
            PipelineImageService.generate_download_cover_promise(data_set.url, data_set._type)
                .then(function(resp){
        ...
        it('Should fail when downloading an invalid image from openlibrary', function (done) {
            var data_set = PipelineImageService.generate_openlibrary_data_set('0971633896');
            PipelineImageService.generate_download_cover_promise(data_set.url, data_set._type)
                .fail(function(err){
                    err.should.be.a.Error;
                })
        ...
        it('Should download an image from openlibrary', function (done) {
            var data_set = PipelineImageService.generate_openlibrary_data_set('0971633894');
            PipelineImageService.generate_download_cover_promise(data_set.url, data_set._type)
                .then(function(resp){
                    resp.headers['content-type'].should.eql('image/jpeg');
                })
        ...
        it('Should download an isbn13 from openlibrary when provided one.', function (done) {
            var data_set = PipelineImageService.generate_openlibrary_data_set('9780060529703');
            PipelineImageService.generate_download_cover_promise(data_set.url, data_set._type)
                .then(function(resp){
                    resp.headers['content-type'].should.eql('image/jpeg')
                })
    ...
    describe('#generate_goodreads_data_set()', function () {
        ...
        it('Should fail when downloading an invalid image', function (done) {
            var data_set = PipelineImageService.generate_goodreads_data_set('http://www.example.com/notreal.jpg');
            PipelineImageService.generate_download_cover_promise(data_set.url, data_set._type)
        ...
        it('Should fail when downloading "nophoto" image', function () {
            ...
            should(function(){
                return PipelineImageService.generate_goodreads_data_set('https://s.gr-assets.com/assets/nophoto/book/111x148-6204a98ba2aba2d1aa07b9bea87124f8.png') == null;
            })
        });

        it('Should download an image by url', function (done) {
            var data_set = PipelineImageService.generate_goodreads_data_set('https://d.gr-assets.com/books/1309286860m/1233.jpg');
            PipelineImageService.generate_download_cover_promise(data_set.url, data_set._type)
                .then(function(resp){
                    resp.headers['content-type'].should.eql('image/jpeg')
    ...
    describe.skip('#retrieve_cover_with_fallback()', function(){
        ...
        it('Should fallback from invalid isbn to download goodreads cover', function (done) {
            CoverArtService.retrieve_cover_with_fallback({type:'isbn',isbn: '12345678'},{
                type: 'url',
                source: 'goodreads',
